...
import { addProductToCartAPI, findCartByCustomerId } from '~/apis/clientAPI/cartApi'
...
export default function ProductCardDetail({ product, open, onClose }: ProductCardDetailProps) {

  const accessToken = localStorage.getItem('accessTokenClient') || null
  const user = accessToken ? jwtDecode(accessToken) as { sub: string } : null
  ...
  const [addProductStatus, setAddProductStatus] = useState<'idle' | 'loading' | 'success'>('idle')
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const cart = await findCartByCustomerId(user.sub)
      if (cart) {
      ...
      setTimeout(() => setAddProductStatus('idle'), 3000)
                    ...
                    {activeProductColor.sizes.map((s, idx) => (
                      <div
                        ...
                        className={`
                          ...
                          ${Number(s.quantity) === 0 ? styles.out_of_stock : ''}
                        `}
                        onClick={() => Number(s.quantity) > 0 && setActiveSize(s.size)}
                      >
                        <p>{s.size}</p>
                      </div>
                    ))}
