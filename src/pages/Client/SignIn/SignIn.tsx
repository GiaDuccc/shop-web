...
import '~/App.scss'
import { signInAPI } from '~/apis/clientAPI/authApi'
import { createCartAPI, findCartByCustomerId } from '~/apis/clientAPI/cartApi'
  ...
  const handleSubmit = async () => {
    ...
    await signInAPI(user)
      .then(async (data) => {
        ...
        localStorage.setItem('accessTokenClient', data.accessTokenClient)

        let cart = {}
        cart = await findCartByCustomerId(data.customer._id)
        if (!cart) {
          cart = await createCartAPI(data.customer._id)
        }
        ...
        <div className={`fade-in-up ${styles.content_wrapper}`}>
          <div className={styles.form_container}>
            ...
            <div className={styles.signup_section}>
              <span className={styles.signup_text}>Don&apos;t have an Account?</span>
