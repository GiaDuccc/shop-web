...
import { getCustomerChartByYear } from '~/apis/adminAPI/customerAPI'
import { getOrderChartByYear } from '~/apis/adminAPI/orderAPI'
...
function ChartYear() {
  const [customerData, setCustomerData] = useState<{ month: number, totalCustomers: number }[]>([])
  const [orderData, setOrderData] = useState<{ month: number, totalOrders: number }[]>([])
  const [productData, setProdutData] = useState<{ month: number, totalQuantity: number }[]>([])
  ...
  return (
    <div
      className='chart'
      style={{ width: '100%', display: 'flex', alignItems: 'center', height: '100%', paddingBottom: '8px !important' }}
    >
      <Line
        data={{
