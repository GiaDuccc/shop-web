...
import '~/App.scss'
import { updateProductAPI, uploadImagesAPI, uploadImageAPI } from '~/apis/adminAPI/productAPI'
...
import { Product, ProductColor } from '~/interface/product.interface'
...
export default function EditProduct({ product, open, onClose, refresh }: EditProductProps) {

  const [productColors, setProductColors] = useState<ProductColor[]>(product.colors)
  const [productSizeAndQuantity, setProductSizeAndQuantity] = useState<SizeAndQuantity>({ size: '', quantity: '', id: '' })
  const [adImage, setAdImage] = useState<string | null>(product.adImage)
  const [navbarImage, setNavbarImage] = useState<string | null>(product.navbarImage)
      ...
      <div className={styles.modalBackdrop}>
        ...
        >
                    ...
                    <div className={styles.sizeInputContainer}>
                      ...
                      <div
                        ...
                        onClick={() => {
                          if (!productSizeAndQuantity || !productSizeAndQuantity.size || !productSizeAndQuantity.quantity) return
                          setProductColors((prev: ProductColor[]) => prev.map((color: ProductColor, id: number) =>
                            idx === id ? { ...color, sizes: [...color.sizes || [], { size: productSizeAndQuantity.size, quantity: Number(productSizeAndQuantity.quantity) }].sort((a, b) => Number(a.size) - Number(b.size)) } : color
                          ))
                          setProductSizeAndQuantity({ size: '', quantity: '', id: '' })
                        }}
                      >
          ...
          >
            ...
            {isLoadingAdd === 'failed' && (
              <span className='boom' style={{ display: 'flex', alignItems: 'center' }} >
                <img src={errorIcon} className={styles.statusIcon} />
              </span>
            )}
          </div>
        </div>
