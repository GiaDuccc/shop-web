...
axiosClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    const skipUrls = ["/auth/signInClient", "/auth/refreshTokenClient"];
    ...
    if (status === 401 && !originalRequest._retry) {
      ...
      try {
        const refreshResponse = await axiosClient.post("/auth/refreshTokenClient");
        console.log(refreshResponse)
        const newToken = refreshResponse.data.accessTokenClient;

        // Lưu token mới
        localStorage.setItem("accessTokenClient", newToken);
